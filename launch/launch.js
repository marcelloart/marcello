...
async function connectDetail(detail){try{const p=detail.provider;activeProvider=p;const accounts=await p.request({method:"eth_requestAccounts"});if(!accounts||!accounts[0])throw new Error("Tidak ada akun yang dipilih.");account=accounts[0];await ensureNetwork(p);$("walletPicker").hidden=true;$("walletState").textContent=detail.info.name+" Connected";$("walletState").classList.add("is-ok");$("walletInfo").hidden=false;$("walletAddress").textContent=account;$("connectWallet").textContent=detail.info.name+" Connected";$("deployToken").disabled=false;$("deployToken").textContent="Deploy Token on Testnet →"}catch(e){alert(e.message||"Wallet connection failed")}}
