...
$("tokenForm").addEventListener("submit",async e=>{e.preventDefault();if(!account||!activeProvider)return;const name=$("tokenName").value.trim(),symbol=$("tokenSymbol").value.trim().toUpperCase(),raw=$("tokenSupply").value.trim();if(!name||!symbol||!/^\d+$/.test(raw)){alert("Periksa kembali data token.");return}try{await ensureNetwork();const chainHex=await activeProvider.request({method:"eth_chainId"});if(parseInt(chainHex,16)!==46630)throw new Error("Wallet belum berada di Robinhood Chain Testnet (Chain ID 46630).");const balanceHex=await activeProvider.request({method:"eth_getBalance",params:[account,"latest"]});if(BigInt(balanceHex)===0n)throw new Error("Wallet tidak memiliki ETH Robinhood Testnet untuk membayar gas deployment.");$("launchProgress").hidden=false;$("launchSuccess").hidden=true;$("progressTitle").textContent="Compiling ERC-20…";$("progressText").textContent="Menyiapkan fixed-supply contract di browser Anda.";await new Promise(r=>setTimeout(r,50));const c=compile(name,symbol);const supply=ethers.parseUnits(raw,18);const iface=new ethers.Interface(c.abi);const deployData="0x"+c.evm.bytecode.object+iface.encodeDeploy([name,symbol,supply,account]).slice(2);$("progressTitle").textContent="Confirm deployment in wallet";$("progressText").textContent="Periksa estimasi gas dan konfirmasi transaksi testnet.";let gas;try{gas=await activeProvider.request({method:"eth_estimateGas",params:[{from:account,data:deployData}]})}catch(ge){console.warn("Gas estimate failed",ge)}const tx={from:account,data:deployData};if(gas)tx.gas=gas;const txHash=await activeProvider.request({method:"eth_sendTransaction",params:[tx]});$("progressTitle").textContent="Waiting for confirmation…";$("progressText").textContent="Transaction: "+txHash;const rpc=new ethers.JsonRpcProvider(CHAIN.rpcUrls[0],{name:"robinhood-testnet",chainId:46630},{staticNetwork:true});const receipt=await rpc.waitForTransaction(txHash);if(!receipt||receipt.status!==1)throw new Error("Deployment transaction gagal dikonfirmasi.");const addr=receipt.contractAddress;$("launchProgress").hidden=true;$("launchSuccess").hidden=false;$("contractAddress").textContent=addr;$("explorerLink").href=CHAIN.blockExplorerUrls[0]+"/address/"+addr;$("copyAddress").onclick=()=>navigator.clipboard.writeText(addr);$("launchSuccess").scrollIntoView({behavior:"smooth"})}catch(err){$("launchProgress").hidden=true;console.error("Deploy error:",err);let msg=(err&&err.error&&err.error.message)||err.shortMessage||err.reason||err.message||"Deployment failed";if(err.code==="ACTION_REJECTED"||err.code===4001)msg="Transaksi deployment dibatalkan di wallet.";if(/insufficient funds/i.test(msg))msg="ETH Robinhood Testnet tidak cukup untuk gas deployment.";alert(msg)}});})();